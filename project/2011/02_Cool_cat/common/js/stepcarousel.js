...
var stepcarousel = {
  ajaxloadingmsg:
    '<div style="margin: 1em; font-weight: bold"><img src="ajaxloadr.gif" style="vertical-align: middle" /> Fetching Content. Please wait...</div>', //customize HTML to show while fetching Ajax content
  configholder: {},
  ...
  alignpanels: function ($, config) {
    ...
    config.$panels.each(function (index) {
      ...
      paneloffset +=
        stepcarousel.getCSSValue($currentpanel.css("marginRight")) + parseInt($currentpanel.get(0).offsetWidth); //calculate next panel offset
  ...
  stepTo: function (galleryid, pindex) {
    ...
    config.$belt.animate({ left: -endpoint + "px" }, "slow", function () {
      config.onslideaction(this);
    });
  ...
  stepBy: function (galleryid, steps) {
    ...
    pindex =
      pindex > config.paneloffsets.length - 1 || (pindex < 0 && pindex - steps > 0)
        ? 0
        : pindex < 0
          ? config.paneloffsets.length + steps
          : pindex; //take into account end or starting panel and step direction
  ...
  setup: function (config) {
    ...
    jQuery(document).ready(function ($) {
      ...
      config.onslideaction =
        typeof config.onslide == "undefined"
          ? function () {}
          : function (beltobj) {
              $(beltobj).stop();
              config.onslide();
      ...
      else stepcarousel.alignpanels($, config); //align panels and initialize gallery
    }); //end document.ready
