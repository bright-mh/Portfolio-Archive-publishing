package com.climbx.images {
	 ...
	 */
	public class HiResJPGEncoder extends EventDispatcher {

		/** Pixel density is in pixels per inch. */
		public static const DENSITY_UNIT_DOTS_INCH : uint = 1;
		
		private static const PIXEL_PER_ITERATION:uint = 2880;
		...
		private function writeAPP0(dpi : uint) : void {
			...
			writeByte(DENSITY_UNIT_DOTS_INCH); // xyunits
			writeWord(dpi); // xdensity
			writeWord(dpi); // ydensity
		 ...
		 * @tiptext
		 */
		public function HiResJPGEncoder(quality : Number = 80) {
			if (quality <= 0) {
				quality = 1;
			}
			if (quality > 100) {
		 ...
		 * @tiptext
		 */
		public function encode(image : BitmapData, dpi : uint = 72) : ByteArray {
			// Initialize bit writer
			initializeBitWriter();

			// Add JPEG headers
			addJPEGHeaders(image.width, image.height, dpi);
		...
		public function encodeAsync(image : BitmapData, dpi : uint = 72) : void {
			// Initialize bit writer
			initializeBitWriter();
		...
		private function asyncLoop(image:BitmapData, xpos:int = 0, ypos:int = 0):void {
			...
			setTimeout(asyncLoop, 10, image, xpos, ypos);
		}
		
		private function initializeBitWriter():void {
			byteout = new ByteArray();
			bytenew = 0;
			bytepos = 7;
		}
		
		private function addJPEGHeaders(width:int, height:int, dpi:uint):void {
		...
		private function writeEOI() : void {
