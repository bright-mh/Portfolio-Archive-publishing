...
function fakeselect(v) {
  ...
  function rc(sel, v, f) {
    ...
    if (isie) {
      try {
        occk = false;
        rv = document.createElement(
          "<" +
            tagname +
            ' class="' +
            (v.classname ? (sel.className ? v.classname + " " + sel.className : v.classname) : "") +
            '" style="' +
            style +
            '">',
        );
  ...
  function create(sel) {
    ...
    if (!sel.multiple) {
      ...
      ac.onclick = function () {
        ...
        for (var i = 0, max = opts.length; i < max; i++)
          opts[i].className = i == sel.selectedIndex ? v.option.onclassname : "";
        ...
        this.opt.style.top = isupper
          ? top - this.opt.offsetHeight + v.option.upperposition + "px"
          : top + this.offsetHeight + v.option.position + "px";
      ...
      sel.onkeydown = function (e) {
        ...
        if (e.keyCode == 40 || e.keyCode == 38) {
          if (e.keyCode == 40) {
            f == "down";
            ac.clickindex =
              ac.clickindex == ac.opts.length - 1
                ? ac.opts.length - 1
          ...
          var tnb = gettnbindex();
          var sv =
            tnb[0] > ac.clickindex ? ac.clickindex - tnb[0] : ac.clickindex > tnb[1] ? ac.clickindex - tnb[1] : 0;
    ...
    ac.ckdisable = function () {
      if (sel.disabled) {
        ...
        if (!sel.multiple && v.title.disabledclassname)
          this.className = this.className.replace(new RegExp("\\b" + v.title.disabledclassname + "\\b", "g"), "");
        else if (sel.multiple && v.multiple.disabledclassname)
          this.className = this.className.replace(new RegExp("\\b" + v.multiple.disabledclassname + "\\b", "g"), "");
