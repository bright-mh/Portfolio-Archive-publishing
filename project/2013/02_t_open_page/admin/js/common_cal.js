...
function create_single_calendar(id, max_week, min_day) {
  ...
  if (typeof max_week != "undefined") {
    var max_day = 0;
    if (max_week > 0) {
      max_day = max_week * 7 - day_index - 1;
    }
    $(id).datepicker("option", "maxDate", "+" + max_day + "d");
  }

  // if (typeof (noWeekends) != 'undefined') {
  // if (noWeekends) {
  $(id).datepicker("option", "beforeShowDay", function noSundays(date) {
...
function isNumber(input) {
  ...
  if (typeof input === "string") {
    if (input.search(format) === -1) return true;
  } else {
    try {
      if (input.value.search(format) === -1) return true;
    } catch (e) {}
  }
...
function getBeforeDate(endDateID, dis) {
  ...
  if (strDate.charAt(0) === "A") {
    ...
    var yyyymm = plusMonth(lpad(mainDate.getFullYear(), 4, "0") + lpad(mainDate.getMonth() + 1, 2, "0"), term);
    ...
    mainDate.setDate($(endDateID).val().substring(8));
    var term = AddDays.substring(1);
    var yyyymm = minusMonth(lpad(mainDate.getFullYear(), 4, "0") + lpad(mainDate.getMonth() + 1, 2, "0"), term);
  ...
  if (parseInt(CurDay) < 10) CurDay = "0" + parseInt(CurDay);
...
function beforeAddDate(startDateID, endDateID, dis) {
  if (typeof startDateID == "undefined" || typeof endDateID == "undefined" || typeof dis == "undefined") {
    return;
  }
