	...
	$.fn.extend({
		cPEvent : function(){
			...
			op.bind('mousedown', function(){
				...
				if($(this).attr('class')!= 'area'){
					...
					window.open('/html/gp/gp_realtime001.html','_blank','scrollbars=yes,menubar=no,toolbar=no,status=no,location=no,resizable=yes,channelmode=no,directories=no, copyhistory=no,width=990px,height=500px,left=0,top=0');
					_sub.slideUp(200);
				}
				return false;
				/*End // 2014-01-10 군관리 추가 및 수정*/
		...
		bookMarkEvent : function(){
			...
			creaBtn.bind('mousedown mouseover mouseout', function(e){
				if(e.type == "mousedown"){
					var obj = jQuery("<li class='on'><img src='/html/2014/bems/images/common/icon_forder_over.png' alt=''/><input value='새 폴더' /></li>");
					forLi = jQuery('.bookBox .forder>.fBox>ul>li');
