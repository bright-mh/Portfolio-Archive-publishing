var BC = {
  ...
  txt: {
    nm3: {
      text: "N㎥",
      style: {
        //				color: '#777777',  // 2014-01-06 y축 스타일 통일을 위해 주석처리
        //				'font-size': '12px',
        //				'font-weight': 'bold',
  ...
  }, // y축 레이블을 1000 으로 나눈 값으로 표시할 때 (eg: 데이터가 1000g 일때 1kg 으로 표시를 원할 때 사용)
  dateFormat: {
    hour: { hour: "%H" },
    hourMin: { hour: "%H:%M" },
    day: { day: "%e" },
  ...
  spacing: [40, 15, 0, 0], // 2014-01-06 x 축 우측에 텍스트 짤리는거 수정
  ...
  marker01: {
    ...
    radius: 4,
    lineWidth: 0,
    lineColor: null, // 2014-01-07 marker 통일 위해
  },
...
function onChartLoaded(chartId) {
  var chart = jQuery("#" + chartId).highcharts();
  for (i = 0; i < chart.yAxis.length; i++) {
    chart.yAxis[i].setTitle(BC.aAxisStyle);
  }
}
