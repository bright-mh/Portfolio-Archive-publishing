<%@ page language="java" pageEncoding="UTF-8" %>
<!DOCTYPE html>
<html lang="ko">
<head>
	<jsp:include page="/front/en/inc/top.jsp"></jsp:include>
</head>
<body class="main">
	<jsp:include page="/front/en/inc/head.jsp"></jsp:include>
	<div class="article alert" id="content">
		<div class="gray">
			<p>Pages that are optimized for PC</p>
		</div>
		<div class="wrap">
			<p class="blind">TECHNOLOGICAL INNOVATION</p>
			<!-- lnb s -->
			<jsp:include page="/front/en/inc/lnb_07rnd.jsp"></jsp:include>
			<!-- lnb e -->

			<!-- container s -->
			<div class="s_container">
				<div class="location bg2">
					<ul>
						<li><a href="/front/en/index.jsp"><span class="ico"></span></a></li>
						<li class="gt"><a href="#none">기술혁신</a></li>
						<li class="gt last"><a href="#none">지적재산권</a></li>
					</ul>
				</div>

				<div class="contents">
					<h1>지적재산권</h1>
					<div class="tab tab3">
						<ul>
							<li><a href="/front/en/07rnd/new_techbuild.jsp"><span>신기술</span></a></li>
							<li class="on"><a href="/front/en/07rnd/knowledge_01.jsp"><span>특허</span></a></li>
						</ul>
					</div>
					
					<div class="tab_mn2">
						<ul>
							<li><a href="/front/en/07rnd/knowledge_01.jsp">특허등록현황</a></li>
							<li><a href="/front/en/07rnd/knowledge_02.jsp">실용실안등록현황</a></li>
							<li class="on"><a href="/front/en/07rnd/knowledge_03.jsp">의장등록현황</a></li>
						</ul>
					</div>
					
					<div class="rd table">
						<div class="tbl_red">
						<table summary="명칭, 등록일, 등록번호, 존속기간(10년), 권리자 및 비고">
						<caption>의장등록현황</caption>
						<colgroup>
						<col>
						<col style="width:10%">
						<col style="width:15%">
						<col style="width:20%">
						<col style="width:20%">
						</colgroup>
						<thead>
						<tr>
							<th scope="col">명칭</th>
							<th scope="col">등록일</th>
							<th scope="col">등록번호</th>
							<th scope="col">존속기간(10년)</th>
							<th scope="col">권리자 및 비고</th>
						</tr>
						</thead>
						<tbody>
						<tr>
							<td class="txtL">파이프 연결용 슬리이브</td>
							<td>91.07.16</td>
							<td>173629</td>
							<td>05.12.27</td>
							<td class="txtL">-</td>
						</tr>
						<tr>
							<td class="txtL">글로브 밸브</td>
							<td>91.10.18</td>
							<td>190164</td>
							<td>06.12.09</td>
							<td class="txtL">-</td>
						</tr>
						<tr>
							<td class="txtL">바닥배수용 중간트랩</td>
							<td>94.03.07</td>
							<td>103358</td>
							<td>07.11.10</td>
							<td class="txtL">-</td>
						</tr>
						<tr>
							<td class="txtL">바닥배수용 중간트랩</td>
							<td>95.11.08</td>
							<td>133919</td>
							<td>07.11.10</td>
							<td class="txtL">-</td>
						</tr>
						<tr>
							<td class="txtL">바닥배수욜 콘크리트 거푸집</td>
							<td>97.12.22</td>
							<td>169364</td>
							<td>07.11.24</td>
							<td class="txtL">-</td>
						</tr>
						<tr>
							<td class="txtL">오니준설선의 오니펌프</td>
							<td>98.12.29</td>
							<td>141052</td>
							<td>08.12.22</td>
							<td class="txtL">공동(의성개발)</td>
						</tr>
						<tr>
							<td class="txtL">이원화 온돌 난방배관구조</td>
							<td>00.02.23</td>
							<td>190742</td>
							<td>12.03.30</td>
							<td class="txtL">단독</td>
						</tr>
						</tbody>
						</table>
						</div>
					</div>

					<div class="pagenate">
						<div class="paging">
							<a href="#FirstPage" class="first">첫 페이지</a>
							<a href="#PreviousPage" class="prev">이전 페이지</a>
							<ol>
								<li><a href="#none" class="on">1</a></li>
								<li><a href="#none">2</a></li>
								<li><a href="#none">3</a></li>
								<li><a href="#none">4</a></li>
								<li><a href="#none">5</a></li>
								<li><a href="#none">6</a></li>
								<li><a href="#none">7</a></li>
								<li><a href="#none">8</a></li>
								<li><a href="#none">9</a></li>
							</ol>
							<a href="#NextPage" class="next">다음 페이지</a>
							<a href="#EndPage" class="end">끝 페이지</a>
						</div>
					</div>

				</div>

				<div class="bt_top">
					<a href="#layoutWrap"><img src="/images/sub/bt_top.png" alt="Top of page"/></a>
				</div>
			</div>
			<!-- container e -->

		</div>
	</div>

<jsp:include page="/front/en/inc/footer.jsp"></jsp:include>
</body>
</html>
