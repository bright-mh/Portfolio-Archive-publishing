...
(function ($) {
  ...
  $.tiny.scrollbar = {
    options: {
      ...
      sizethumb: 13, // set the size of the thumb to auto or a fixed number.
  ...
  function Scrollbar(root, options) {
    ...
    this.update = function (sScroll) {
      ...
      iScroll =
        sScroll === "relative" && oContent.ratio <= 1
          ? Math.min(oContent[options.axis] - oViewport[options.axis], Math.max(0, iScroll))
          : 0;
      iScroll =
        sScroll === "bottom" && oContent.ratio <= 1
          ? oContent[options.axis] - oViewport[options.axis]
          : isNaN(parseInt(sScroll, 10))
            ? iScroll
            : parseInt(sScroll, 10);
      ...
      if (oContent.ratio < 1) {
        if (options.invertscroll && touchEvents) {
          iPosition.now = Math.min(
            ...
            Math.max(0, iPosition.start + (iMouse.start - (sAxis ? event.pageX : event.pageY))),
          );
        } else {
          iPosition.now = Math.min(
            oTrack[options.axis] - oThumb[options.axis],
